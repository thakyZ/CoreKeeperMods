...
namespace MoreCommands.Data.Configuration {
  public class JsonConfigFile<T> where T : IConfiguration {
    ...
    public T Context {
      get {
        return context ??= GetDefault.Invoke();
      }
    }
    ...
    /// <inheritdoc cref="JsonConfigFile{T}(string,bool,PugMod.LoadedMod?,System.Func{T})" />
    public JsonConfigFile(string configPath, bool saveOnInit, Func<T> getDefault) : this(configPath, saveOnInit, null, getDefault) { }
    ...
    /// <param name="getDefault">Function to construct a default config object when the context is null.</param>
    public JsonConfigFile(string configPath, bool saveOnInit, LoadedMod? ownerMetadata, Func<T> getDefault) {
    ...
    public void Reload() {
      ...
      try {
        ...
        Logger.Error(
          $"Failed to deserialize the json object for context of type {typeof(T)}\n{exception.Message}\n{exception.StackTrace}");
        return;
      }
    ...
    public void Save() {
      ...
      try {
        ...
        Logger.Error(
          $"Failed to serialize the json object for context of type {typeof(T)}\n{exception.Message}\n{exception.StackTrace}");
        return;
      }
